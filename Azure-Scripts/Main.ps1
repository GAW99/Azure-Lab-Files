...
if($Informative -or $Detailed) { Write-host "Installing the certificate with friendly name: $CertFriendlyName on the Exchange Edge Servers..."  -ForegroundColor Yellow}
...
foreach($UCEdgeServer in $UCEdgeServers)
{
    ...
    Install-LECertificateLegacy -Informative:$Informative -Detailed:$Detailed -RootSharePath $DomainCertificateVault -Creds $UserCredential -PFXFileName ((Get-Item $UCEdgeCertificateData.PfxFile).Name) -PasswordFileName $PasswordFileName -ComputerFQDN $CurrentServerFQDN
}
